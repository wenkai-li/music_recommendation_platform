  ...
  <div class="play-bar" :class="{ show: !toggle }">
    ...
    <div class="control-box">
      ...
      <div class="song-ctr song-edit">
          <div style="margin-left: -40px; color: #4f077c; font-size: 16px" v-if="avgrank">均分: {{parseFloat(avgrank).toFixed(2)}}</div>
              <el-rate style="margin-left: 50px; margin-right: 20px" allow-half v-model="score" :disabled="disabledRank" @click="pushRating()"></el-rate>
        <!--收藏-->
        <yin-icon
          class="yin-play-show"
  ...
  setup() {
    ...
    async function initRating(){
        if(!checkStatus(false)){
            await getRank(songId)
            return;
        }
        await getRank(songId);
        await getUserRank(userId, songId);
    }
    ...
    async function changeCollection() {
      ...
      params.append("type", "0"); // 0 代表歌曲， 1 代表歌单
      params.append("songId", songId.value);
      avgrank.value = ((await HttpManager.getAvgRankOfSong(userId.value)))
    ...
    // 平均评分
    async function getRank(songId) {
        const result = ((await HttpManager.getAvgRankOfSong(songId.value))as ResponseBody);
        avgrank.value = result.data;
    }

    // 获取用户评分
    async function getUserRank(userId, songId) {
        const result = (await HttpManager.getUserRankForSong(userId.value, songId.value)) as ResponseBody;
      ...
      async function pushRating() {
          ...
          nparams.append("uid", userId.value);
          nparams.append("mid", songId.value);
          nparams.append("score", (score.value * 2).toString());

          try {
              const result = (await HttpManager.setSongRank(nparams)) as ResponseBody;
              (proxy as any).$message({
                  message: result.message,
                  type: result.type,
              });
    ...
    onMounted(() => {
      if(songId.value) {
          initCollection();
          getUserRank(userId,songId);
          getRank(songId);
      }

    });

    return { isCollection, playMusic, routerManager, checkStatus, attachImageUrl: HttpManager.attachImageUrl, changeCollection, downloadMusic,avgrank, score, disabledRank, pushRating};
  },
  data() {
    return {
      startTime: "00:00",
