...
const HttpManager = {
  // 获取图片信息
  attachImageUrl: (url) => url ? `${getBaseURL()}/${url}` : "https://cube.elemecdn.com/9/c2/f0ee8a3c7c9638a54940382568c9dpng.png",
  // =======================> 用户 API
  // 登录
  ...
  // 根据不同推荐方法获取歌曲
  getSongByMethod: (method) => get(`song/recommendation/detail?method=${method}`),
  //结合用户进行歌曲推荐
  getSongByUid: (uid, method) => get(`song/recommendation/self?uid=${uid}&&method=${method}`),
  // 返回标题包含文字的歌单
  getSongListOfLikeTitle: (keywords) => get(`songList/likeTitle/detail?title=${keywords}`),
  // 返回歌单里指定歌单ID的歌曲
  getListSongOfSongId: (songListId) => get(`listSong/detail?songListId=${songListId}`),
  //返回全部歌曲
  getAllSong:() => get("song/recommendation"),
  // =======================> 歌手 API
  // 返回所有歌手
  ...
  // =======================> 歌曲评分 API
  //提交歌曲评分
  setSongRank:(params) => post(`rank/add`, params),
  //获得指定歌曲的平均评分
  getAvgRankOfSong: (songId) => get(`rank?songId=${songId}`),
  //获得指定用户和指定歌曲的评分
  getUserRankForSong:(uid, mid) => get(`/rank/user?uid=${uid}&mid=${mid}`),
