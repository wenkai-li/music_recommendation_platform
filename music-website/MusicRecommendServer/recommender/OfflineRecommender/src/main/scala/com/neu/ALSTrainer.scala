...
import com.neu.OfflineRecommender.MONGODB_RATING_COLLECTION
...
object ALSTrainer {
  def main(args: Array[String]): Unit = {
    ...
    val ratingRDD = spark.read
      ...
      .as[MusicRating]
      .rdd
      .map(rating => Rating(rating.uid, rating.mid, rating.score))  //转换成rdd，只保留需要的数据即删除时间戳
      .cache()
  ...
  def adjustALSParam(trainData: RDD[Rating], testData: RDD[Rating]): Unit ={
    val result = for( rank <- Array(20, 50, 100,200); lambda <- Array( 0.001, 0.01, 0.1, 1 ))
      yield {
